...
public class Vitality implements Infusion {
	...
	public static final double HEALTH_MOD_PER_LEVEL = 0.01;
	...
	public void onEquipmentUpdate(Plugin plugin, Player player) {
		...
		if (plugin.mItemStatManager.getInfusionLevel(player, InfusionType.VITALITY) > 0) {
			if (player != null) {
				double healthBoostPct = HEALTH_MOD_PER_LEVEL * plugin.mItemStatManager.getInfusionLevel(player, InfusionType.VITALITY);
