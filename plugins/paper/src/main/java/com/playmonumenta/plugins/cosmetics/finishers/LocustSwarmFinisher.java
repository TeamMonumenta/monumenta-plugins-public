...
public class LocustSwarmFinisher implements EliteFinisher {
	...
	public void run(Player p, Entity killedMob, Location loc) {
		...
		// Bleed
		world.playSound(loc, Sound.ENTITY_VILLAGER_DEATH, SoundCategory.PLAYERS, 0.75F, 0.5F);
		world.playSound(loc, Sound.ENTITY_FOX_BITE, SoundCategory.PLAYERS, 0.8f, 0.5f);
		world.playSound(loc, Sound.ENTITY_WITHER_HURT, SoundCategory.PLAYERS, 0.4f, 0.5f);
		world.playSound(loc, Sound.ENTITY_SHULKER_SHOOT, SoundCategory.PLAYERS, 0.6f, 0.4f);
		world.playSound(loc, Sound.ENTITY_SLIME_DEATH, SoundCategory.PLAYERS, 0.75F, 0.5F);
		world.playSound(loc, Sound.ENTITY_SPIDER_STEP, SoundCategory.PLAYERS, 0.75F, 0.5F);
		...
		new BukkitRunnable() {
			...
			public void run() {
				if (mTicks < 10) {
					world.playSound(centered, Sound.ENTITY_VEX_DEATH, 0.75F, 2F);
					world.playSound(centered, Sound.BLOCK_SAND_PLACE, 0.75F, 0.8F);
					for (int i = 0; i < 12; i++) {
					...
					new BukkitRunnable() {
						...
						public void run() {
							if (mT < 20) {
								...
								this.cancel();
								world.playSound(centered, Sound.ENTITY_VEX_DEATH, 1F, 1.5F);
							}
							mT++;
						}
