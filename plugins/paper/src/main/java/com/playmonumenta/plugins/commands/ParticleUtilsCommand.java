...
public class ParticleUtilsCommand {
	...
	public static void register() {
		...
		new CommandAPICommand(COMMAND)
			...
			.withArguments(
				...
				new StringArgument("particle").includeSuggestions(
					ArgumentSuggestions.strings(Arrays.stream(Particle.values()).map(Enum::name).collect(Collectors.toList()))
				),
				new DoubleArgument("distance falloff"),
				new TextArgument("extra data").includeSuggestions(
	...
	private static void spawnAsActive(CommandSender callee, AbstractPartialParticle<? extends AbstractPartialParticle<?>> particle) {
		if (callee instanceof Player player) {
			...
			particle.spawnAsEntityActive((Entity) callee);
		}
	}

	private static void doLine(CommandSender callee, Location start, Location end, double countPerMeter, Particle particle, double distanceFalloff, @Nullable Object data) {
	...
	private static void doRectangle(CommandSender callee, Location start, double dx, double dz, double countPerMeter, Particle particle, double distanceFalloff, @Nullable Object data) {
		PPLine[] sides = {
	...
	private static void doCircle(CommandSender callee, boolean ringMode, Location center, double radius, double countPerMeter, Particle particle, double distanceFalloff, @Nullable Object data) {
	...
	private static void doCircleTelegraph(CommandSender callee, Location center, double radius, int count, Particle particle, double distanceFalloff, double particleSpeed, int pulses, int telegraphDuration, int pulseStartOffset, @Nullable Object data) {
		// Input Validation
