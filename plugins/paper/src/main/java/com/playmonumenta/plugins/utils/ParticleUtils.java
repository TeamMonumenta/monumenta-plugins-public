...
public class ParticleUtils {
	...
	public static void drawLine(Location start, int units, double spacing, Vector direction, Particle particle) {
		Location current = start.clone();
		Vector increment = direction.normalize().multiply(spacing);
		for (int i = 0; i < units; i++) {
			new PartialParticle(particle, current, 1).extra(0).spawnFull();
