...
public class SpiritualCombosCS implements CosmeticSkill {

	@Override
	public ClassAbility getAbility() {
		return ClassAbility.SPIRITUAL_COMBOS;
	}
	...
	private static final Particle.DustOptions PARTICLE_COLOR = new Particle.DustOptions(Color.fromRGB(255, 230, 150), 0.8f);
	...
	public void spiritualCombosSwirl(List<PPPeriodic> mParticles, Player player) {
		mParticles.add(new PPPeriodic(Particle.REDSTONE, player.getLocation()).extra(0.1).data(new Particle.DustOptions(Color.fromRGB(255, 200, 100), 0.8f)).spawnAsPlayerPassive(player));
	}

	public void spiritualCombosTrigger(Player player) {
		player.getWorld().playSound(player.getLocation(), Sound.BLOCK_BEACON_ACTIVATE, SoundCategory.PLAYERS, 2, 1.5f);
	}

	public void spiritualCombosActiveSwirl(List<PPPeriodic> mParticles, Player player, double mRotationAngle, boolean mSpendingStacks, int i) {
		...
		particle.location(LocationUtils.getHalfHeightLocation(player)
			...
			.data(new Particle.DustOptions(Color.fromRGB(255, mSpendingStacks ? 230 : 200, mSpendingStacks ? 150 : 100), 0.8f))
			.spawnAsPlayerPassive(player);
	}

	public void spiritualCombosHit(Player player, LivingEntity target) {
		...
		new PPLine(Particle.REDSTONE,
			...
			LocationUtils.getHalfHeightLocation(target)).data(new Particle.DustOptions(Color.fromRGB(255, 200, 200), 0.8f))
			.delta(0.1).extra(0.1).countPerMeter(10)
			.spawnAsPlayerActive(player);
	}

	public void spiritualCombosExpire(Player player, Plugin plugin) {
