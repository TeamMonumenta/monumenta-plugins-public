...
public class EnchantedPrayerCS implements CosmeticSkill {
	...
	public void onCast(Plugin plugin, Player player, World world, Location loc) {
		...
		new BukkitRunnable() {
			...
			public void run() {
				mRadius += 0.5;
				new PPCircle(Particle.SPELL_INSTANT, mLoc, mRadius).count(35).delta(0.15).spawnAsPlayerActive(player);
				if (mRadius >= 5) {
					this.cancel();
				}
			}
	...
	public void onEffectTrigger(Player player, Player cleric, World world, Location loc, LivingEntity enemy, double mEffectSize) {
		...
		new PartialParticle(Particle.SPELL_INSTANT, loc.clone().add(0, enemy.getHeight() / 2, 0), 50, 0.25f, 0.3f, 0.25f, 1).spawnAsPlayerActive(player);
		new PartialParticle(Particle.FIREWORKS_SPARK, loc.clone().add(0, enemy.getHeight() / 2, 0), 30, 0, 0, 0, 0.3).spawnAsPlayerActive(player);

		new PPCircle(Particle.SPELL_INSTANT, loc.clone().add(0, 0.2, 0), 0.1)
			.rotateDelta(true)
		...
		new BukkitRunnable() {
			...
			public void run() {
				mTicks++;
				new PPCircle(Particle.ENCHANTMENT_TABLE, loc.clone().add(0, 1, 0), mEffectSize * mTicks / 4)
					.count(20)
					.extra(1)
					.spawnAsPlayerActive(player);

				new PPCircle(Particle.SPELL_INSTANT, loc.clone().add(0, 0.2, 0), mEffectSize * mTicks / 4)
					.count(50)
					.extra(0.1)
					.spawnAsPlayerActive(player);
				if (mTicks > 4) {
