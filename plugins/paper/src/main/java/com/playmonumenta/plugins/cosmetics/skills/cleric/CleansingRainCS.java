...
public class CleansingRainCS implements CosmeticSkill {
	...
	public void rainCast(Player player) {
		World world = player.getWorld();
		world.playSound(player.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, SoundCategory.PLAYERS, 1.45f, 0.8f);
	}

	public void rainCloud(Player player, double ratio) {
		new PartialParticle(Particle.CLOUD, player.getLocation().add(0, 4, 0), (int) (5 * ratio * ratio), 2.5 * ratio, 0.35, 2.5 * ratio, 0).spawnAsPlayerActive(player);
	...
	public void rainEnhancement(Player player, double smallRatio) {
