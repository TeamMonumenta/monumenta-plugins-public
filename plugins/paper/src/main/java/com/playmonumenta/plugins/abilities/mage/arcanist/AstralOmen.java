	...
	public boolean onDamage(DamageEvent event, LivingEntity enemy) {
		...
		if (combo >= stacksThreshold) { // Adding 1 more stack would hit threshold, which removes all stacks anyway, so don't bother adding then removing
			...
			if (mLevelBonusMultiplier > 0) {
				mPlugin.mEffectManager.addEffect(enemy, BONUS_DAMAGE_SOURCE,
					new AstralOmenBonusDamage(BONUS_TICKS, mLevelBonusMultiplier, mPlayer, mCosmetic).deleteOnAbilityUpdate(true));
			}
