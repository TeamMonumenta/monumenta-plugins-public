...
public class TransmutationRing extends PotionAbility {
	...
	public TransmutationRing(Plugin plugin, @Nullable Player player) {
		...
		mInfo.mDescriptions.add("Right click while sneaking and holding an Alchemist's Bag to create a Transmutation Ring at the potion's landing location that lasts for 10 seconds. The ring has a radius of 5 blocks. Other players within this ring receive 15% extra damage on all sources. Mobs that die within this ring increase the damage bonus by 1% per mob, up to 30% total extra damage. Cooldown: 25s.");
