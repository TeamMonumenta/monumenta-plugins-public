...
public class BrutalAlchemy extends Ability implements PotionAbility {
	...
	public static final AbilityInfo<BrutalAlchemy> INFO =
		new AbilityInfo<>(BrutalAlchemy.class, "Brutal Alchemy", BrutalAlchemy::new)
			...
			.descriptions(
				...
				"as magic damage every second, over %ss. The damage dealt by this effect cannot be less than 1.")
					.formatted(
	...
	public void apply(LivingEntity mob, boolean isGruesome, ItemStatManager.PlayerItemStats playerItemStats) {
		if (!isGruesome && mAlchemistPotions != null) {
			...
			CustomDamageOverTime dot = new CustomDamageOverTime(duration, Math.max(1, CharmManager.calculateFlatAndPercentValue(mPlayer, CHARM_DOT_DAMAGE, finalDamage)), mPeriod, mPlayer, playerItemStats, mInfo.getLinkedSpell(), DamageEvent.DamageType.MAGIC);
