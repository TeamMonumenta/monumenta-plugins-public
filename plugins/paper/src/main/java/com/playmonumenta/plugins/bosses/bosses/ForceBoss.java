...
public class ForceBoss extends BossAbilityGroup {
	...
	public static class Parameters {
		...
		/** Effects applied to the player if he is near the boss (< RADIUS /3) when cast is over */
		public EffectsList EFFECTS_NEAR = EffectsList.fromString("[(pushforce,3),(SLOW,100,2)]");
		/** Effects applied to the player if he is near the boss (< Radius * 2/3) when cast is over */
		public EffectsList EFFECTS_MIDDLE = EffectsList.fromString("[(pushforce,2.1),(SLOW,100,1)]");
		/** Effects applied to the player if he is at limit distance to the boss (< Radius) when cast is over */
		public EffectsList EFFECTS_LIMIT = EffectsList.fromString("[(pushforce,1.2),(SLOW,100,0)]");

		//Particles & Sounds!
		/** Particle summon in the air while the ability is charging */
		public ParticlesList PARTICLE_AIR = ParticlesList.fromString("[(SMOKE_LARGE,1)]");
		/** Particle summon int the ground while the ability is charging */
		public ParticlesList PARTICLE_CIRCLE = ParticlesList.fromString("[(CRIT_MAGIC,1)]");
		/** Particle when the ability explode */
		...
		/** Particle summon at player position when he got hit by the ability */
