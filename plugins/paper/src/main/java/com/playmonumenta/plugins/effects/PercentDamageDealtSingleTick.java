...
public final class PercentDamageDealtSingleTick extends PercentDamageDealt {
	...
	private int mDurationRemainingWhenTriggered;
	...
	public static @Nullable PercentDamageDealtSingleTick deserialize(JsonObject object, Plugin plugin) {
		...
		if (hasDoneDamage) {
			...
			if (object.has("type")) {
				...
				return (PercentDamageDealtSingleTick) new PercentDamageDealtSingleTick(duration, amount).damageTypes(damageTypeSet);
			} else {
				return new PercentDamageDealtSingleTick(duration, amount);
			}
		}
	}
