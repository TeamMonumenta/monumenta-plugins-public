		...
		new AbilityInfo<>(EmpoweringOdor.class, "Empowering Odor", EmpoweringOdor::new)
			...
			.descriptions(
				"Other players hit by your Alchemist's Potions are given %s%% speed and %s%% damage for %ss."
					.formatted(
						StringUtils.multiplierToPercentage(SPEED_POTENCY),
						StringUtils.multiplierToPercentage(DAMAGE_POTENCY),
				...
				"refreshing on each application.")
					.formatted(StringUtils.multiplierToPercentage(ENHANCE_DAMAGE_POTENCY))
			)
