...
public class ThornsDamage implements Attribute {
	...
	@Override
	public void onHurtByEntityWithSource(Plugin plugin, Player player, double value, DamageEvent event, Entity damager, LivingEntity source) {
		...
		if (value > 0 && (event.getType() == DamageType.MELEE || event.getType() == DamageType.PROJECTILE) && !event.isCancelled() && !event.isBlocked()) {
