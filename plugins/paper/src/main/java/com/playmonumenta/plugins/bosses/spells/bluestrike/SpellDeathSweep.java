...
public class SpellDeathSweep extends Spell {
	...
	public SpellDeathSweep(Plugin plugin, LivingEntity boss, int phase) {
		...
		mPRed = new PartialParticle(Particle.REDSTONE, mBoss.getLocation(), 1, 0.1, 0.1, 0.1, 0.1, RED).minimumMultiplier(false);
		mPHitCone1 = new PartialParticle(Particle.SPELL_WITCH, mBoss.getLocation(), 1, 0.1, 0.1, 0.1, 0.1).minimumMultiplier(false);
		mPHitCone2 = new PartialParticle(Particle.CRIT_MAGIC, mBoss.getLocation(), 2, 0.1, 0.1, 0.1, 0.1).minimumMultiplier(false);
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if (mT % (getCastTime() / 4) == 0) {
					mBoss.getWorld().playSound(mBoss.getLocation(), Sound.ENTITY_ZOMBIE_ATTACK_WOODEN_DOOR, SoundCategory.HOSTILE, 5, 1);
				}

				if (mT > mDelay) {
					mBoss.setAI(true);
					mBoss.getWorld().playSound(mBoss.getLocation(), Sound.ENTITY_ZOMBIE_BREAK_WOODEN_DOOR, SoundCategory.HOSTILE, 5, 0.7f);
