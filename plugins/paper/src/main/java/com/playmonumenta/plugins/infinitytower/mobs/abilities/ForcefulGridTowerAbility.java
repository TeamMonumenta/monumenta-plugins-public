...
public class ForcefulGridTowerAbility extends TowerAbility {
	public ForcefulGridTowerAbility(Plugin plugin, String identityTag, LivingEntity boss, TowerGame game, TowerMob mob, boolean isPlayerMob) {
		...
		if (isPlayerMob) {
			targets = new EntityTargets(EntityTargets.TARGETS.MOB, 100, new EntityTargets.Limit(1, EntityTargets.Limit.SORTING.FARTHER), List.of(), new EntityTargets.TagsListFiter(new HashSet<>(List.of(TowerConstants.MOB_TAG_FLOOR_TEAM))), false);
		} else {
			targets = new EntityTargets(EntityTargets.TARGETS.MOB, 100, new EntityTargets.Limit(1, EntityTargets.Limit.SORTING.FARTHER), List.of(), new EntityTargets.TagsListFiter(new HashSet<>(List.of(TowerConstants.MOB_TAG_PLAYER_TEAM))), false);
		}
