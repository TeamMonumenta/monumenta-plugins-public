...
public class RegionScalingDamageDealt implements Enchantment {
	...
	public void onDamage(Plugin plugin, Player player, double value, DamageEvent event, LivingEntity enemy) {
		...
		event.setFlatDamage(event.getFlatDamage() * DAMAGE_DEALT_MULTIPLIER[Math.max(0, Math.min((int) value, DAMAGE_DEALT_MULTIPLIER.length - 1))]);
	}

	@Override
	public void tick(Plugin plugin, Player player, double value, boolean twoHz, boolean oneHz) {
		ItemStack item = player.getInventory().getItemInMainHand();
		if (ItemStatUtils.getAttributeAmount(item, AttributeType.ATTACK_DAMAGE_ADD, Operation.ADD, Slot.MAINHAND) > 0 || ItemUtils.isPickaxe(item) || ItemUtils.isAxe(item) || ItemUtils.isShovel(item)) {
