...
public class IceBarrier extends DepthsAbility {
	...
	public boolean cast() {
		...
		DepthsUtils.freezeTerrain(block, CAST_TIME, false, false);
		...
		if (!mIsPrimed || mPrimedLoc == null) {
			...
			new BukkitRunnable() {
				...
				public void run() {
					if (mIsPrimed && mPrimedLoc != null) {
						...
						//Reset cd
						mPlugin.mTimers.setCooldown(mPlayer, ClassAbility.ICE_BARRIER, 0);
					}
				}
			...
			for (Block b : blocksToIce) {
				...
				DepthsUtils.freezeTerrain(b.getRelative(BlockFace.UP), mIceDuration, true, false);
