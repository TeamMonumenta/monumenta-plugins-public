...
public class Meowing implements Enchantment {
	...
	public EnchantmentType getEnchantmentType() {
		return EnchantmentType.MEOWING;
	}
	...
	public void tick(Plugin plugin, Player player, double value, boolean twoHz, boolean oneHz) {
		...
		if (oneHz && mRun) {
			mRun = false;
			player.getWorld().playSound(player.getLocation(), Sound.ENTITY_CAT_AMBIENT, SoundCategory.PLAYERS, 1.0f, 0.8f);
		} else if (oneHz) {
			mRun = true;
		}
	}
	...
	public void onSpawn(Plugin plugin, Item item, double value) {
		new BukkitRunnable() {
			...
			public void run() {
				...
				item.getWorld().playSound(item.getLocation(), Sound.ENTITY_CAT_AMBIENT, SoundCategory.PLAYERS, 1.0f, 0.8f);
