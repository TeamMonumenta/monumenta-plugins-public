...
public class TranscCombosCS extends ViciousCombosCS {
	...
	public void comboOnKill(World world, Location loc, Player player, double range, LivingEntity target) {
		...
		new PartialParticle(Particle.CRIT_MAGIC, eLoc, 50, 0, 0, 0, 1).spawnAsPlayerActive(player);
		new PartialParticle(Particle.END_ROD, eLoc, 10, 0, 0, 0, 0.15).spawnAsPlayerActive(player);

		drawX(eLoc, player, LINE_LENGTH, TRANSC_LINE_COLOR);
	}
	...
	public void comboOnElite(World world, Location loc, Player player, double range, LivingEntity target) {
		...
		drawX(eLoc, player, LINE_LENGTH + 0.75, TRANSC_LINE_ELITE_COLOR);

		new PartialParticle(Particle.END_ROD, eLoc, 15, 0, 0, 0, 0.15).spawnAsPlayerActive(player);
		new PartialParticle(Particle.CRIT_MAGIC, eLoc, 50, 0, 0, 0, 1).spawnAsPlayerActive(player);
		Location finalLoc = loc;
		new BukkitRunnable() {
			...
			public void run() {
				...
				for (int i = 0; i < 6; i++) {
					createRandomLine(finalLoc, player);
				}
				if (mT < 5) {
					...
					ParticleUtils.drawParticleLineSlash(eLoc, VectorUtils.rotateTargetDirection(player.getLocation().getDirection(), 90, 15),
					...
					for (double speed = 0; speed < 0.7; speed += 0.02) {
						rotation += 3.5;
						ParticleUtils.drawParticleCircleExplosion(player, cLoc, 0, 1, 0, 0, 3, (float) speed,
							true, rotation, 0, Particle.END_ROD);
					}

					Location pLoc = player.getLocation().add(0, 3.5, 0);
					new PartialParticle(Particle.END_ROD, pLoc, 35, 0, 0, 0, 0.15).spawnAsPlayerActive(player);
					pLoc.setPitch(0);
					ParticleUtils.drawParticleLineSlash(pLoc, VectorUtils.rotateTargetDirection(pLoc.getDirection(), 90, 15),
						...
						(Location lineLoc, double middleProgress, double endProgress, boolean middle) -> {
							...
							new PartialParticle(Particle.REDSTONE, lineLoc, 3, 0.05, 0.05, 0.05, 0.25,
								new Particle.DustOptions(Color.fromRGB(255, 255, 255), size)).spawnAsPlayerActive(player);
						});

					ParticleUtils.drawParticleLineSlash(pLoc, VectorUtils.rotateTargetDirection(pLoc.getDirection(), 90, 15),
						0, 5, 0.2, 5,
						(Location lineLoc, double middleProgress, double endProgress, boolean middle) -> {
							new PartialParticle(Particle.END_ROD, lineLoc, 2, 0, 0, 0, 0).spawnAsPlayerActive(player);
						});
				}
