...
public class SpellFrostNova extends SpellBaseAoE {
	...
	protected void chargeCircleAction(Location loc, double radius) {
		new PPCircle(Particle.SNOWBALL, loc, radius).count(12).delta(0.25).spawnAsEntityActive(mLauncher);
	}
	...
	protected void circleOutburstAction(Location loc, double radius) {
		new PPCircle(Particle.CLOUD, loc, radius).count(48).delta(0.1).extra(0.2).spawnAsEntityActive(mLauncher);
		new PPCircle(Particle.SNOWBALL, loc, radius).count(24).delta(0.25).spawnAsEntityActive(mLauncher);
	}
