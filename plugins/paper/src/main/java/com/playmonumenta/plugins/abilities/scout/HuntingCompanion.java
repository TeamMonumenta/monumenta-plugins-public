...
public class HuntingCompanion extends Ability {
	...
	public HuntingCompanion(Plugin plugin, @Nullable Player player) {
		...
		mInfo.mDescriptions.add("Also summon an invulnerable eagle (parrot). The eagle deals the same damage as the fox and targets similarly, although the two will always avoid targeting the same mob at once. The eagle applies " + (int)(BLEED_AMOUNT * 100) + "% Bleed for " + BLEED_DURATION / 20 + "s instead of stunning, which can be reapplied on a mob.");
