...
public class PrestigiousPinningShotCS extends PinningShotCS implements PrestigeCS {
	...
	public void pinEffect1(World world, Player player, LivingEntity enemy) {
		...
		Vector mFront = eLoc.toVector().subtract(player.getLocation().toVector()).setY(0).normalize();
	...
	public void pinEffect2(World world, Player player, LivingEntity enemy) {
		...
		new PartialParticle(Particle.FIREWORKS_SPARK, eLoc, 40, 0, 0, 0, 0.4).spawnAsPlayerActive(player);
		new PartialParticle(Particle.REDSTONE, eLoc, 60, 0.75, 0.5, 0.75, 0, LIGHT_COLOR).spawnAsPlayerActive(player);
		Vector mFront = eLoc.toVector().subtract(player.getLocation().toVector()).setY(0).normalize();
		ParticleUtils.drawRing(eLoc, 36, mFront, 1.6,
			(l, t) -> {
