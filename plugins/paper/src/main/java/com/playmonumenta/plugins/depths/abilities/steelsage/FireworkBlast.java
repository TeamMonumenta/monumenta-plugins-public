			...
			public void run() {
				if (rocket.isDead() || !rocket.isValid() || rocket.getVelocity().equals(new Vector(0, 0, 0))) {
					...
					for (LivingEntity e : EntityUtils.getNearbyMobs(rocket.getLocation(), RADIUS)) {
						...
						DamageUtils.damage(mPlayer, e, new DamageEvent.Metadata(DamageType.PROJECTILE_SKILL, mInfo.mLinkedSpell, playerItemStats), Math.min(damage, DAMAGE_CAP[mRarity - 1]), false, true, null);
