	...
	private void animation(Location loc) {
		...
		Runnable anim_loop = new Runnable() {
			...
			public void run() {
				...
				centerLoc.getWorld().playSound(centerLoc, Sound.BLOCK_FIRE_AMBIENT, (float)mRadius / 7, (float)(0.5 + mRand.nextInt(150) / 100));
				centerLoc.getWorld().spawnParticle(Particle.FLAME, centerLoc, 10, 1, 1, 1, 0.01);
			}
		};
		...
		Runnable anim_loop2 = new Runnable() {
			...
			public void run() {
				...
				for (int j = 0; j < precision; j++) {
					...
					particleLoc.getWorld().spawnParticle(Particle.FLAME, particleLoc, 1, 0.02, 1.5 * rad, 0.02, 0);
				}
				if (w == 0) {
