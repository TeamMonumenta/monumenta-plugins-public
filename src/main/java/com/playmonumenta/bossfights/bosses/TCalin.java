...
public class TCalin extends BossAbilityGroup {
	...
	public TCalin(Plugin plugin, LivingEntity boss, Location spawnLoc, Location endLoc) {
		...
		(Location loc) -> {
			...
			new BukkitRunnable() {
				...
				public void run() {
					...
					j++;
					world.spawnParticle(Particle.SPELL_WITCH, mBoss.getLocation().add(0, 1, 0), 2, 0.25, 0.45,
					0.25);
		...
		List<Spell> passiveSpells = Arrays.asList(new SpellBaseAura(mBoss, 8, 5, 8, 10, Particle.FALLING_DUST,
		...
		events.put(100, mBoss -> {
			Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(), Utils.getExecuteCommandOnNearbyPlayers(
			    ...
			    "tellraw @s [\"\",{\"text\":\"[T'Calin] \",\"color\":\"gold\"},{\"text\":\"Fangride! You dare test the might of the boss-coder FirelordWeaponry!? Let me show you his efficiency!\",\"color\":\"white\"}]"));
		});

		events.put(75, mBoss -> {
			Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(), Utils.getExecuteCommandOnNearbyPlayers(
			    spawnLoc, detectionRange,
			    "tellraw @s [\"\",{\"text\":\"[T'Calin] \",\"color\":\"gold\"},{\"text\":\"You're strong, but this fight is only beginning for you!\",\"color\":\"white\"}]"));
		});
		...
		events.put(50, mBoss -> {
			...
			new BukkitRunnable() {
				...
				public void run() {
					...
					if (radius <= 0) {
						...
						changePhase(null, passiveSpells, (LivingEntity entity) -> {
			...
			Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(), Utils.getExecuteCommandOnNearbyPlayers(
			                                       spawnLoc, detectionRange,
			                                       "tellraw @s [\"\",{\"text\":\"[T'Calin] \",\"color\":\"gold\"},{\"text\":\"You do not understand the power my master has bestowed upon me in less than an hour! Let me show you!\",\"color\":\"white\"}]"));
		});
	...
	public void death() {
		Utils.executeCommandOnNearbyPlayers(mBoss.getLocation(), detectionRange,
		                                    "playsound minecraft:entity.wither.death master @s ~ ~ ~ 100 0.8");
		Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(), Utils.getExecuteCommandOnNearbyPlayers(mSpawnLoc,
		                                   detectionRange,
		                                   "tellraw @s [\"\",{\"text\":\"[T'Calin] \",\"color\":\"gold\"},{\"text\":\"Now you understand... Now... Change my... damn dialogue...\",\"color\":\"white\"}]"));
	...
	public void init() {
		...
		Utils.executeCommandOnNearbyPlayers(mBoss.getLocation(), detectionRange,
		                                    "title @s title [\"\",{\"text\":\"T'Calin\",\"color\":\"green\",\"bold\":true}]");
		Utils.executeCommandOnNearbyPlayers(mBoss.getLocation(), detectionRange,
		                                    "title @s subtitle [\"\",{\"text\":\"Forest Battlemage\",\"color\":\"dark_green\",\"bold\":true}]");
		Utils.executeCommandOnNearbyPlayers(mBoss.getLocation(), detectionRange,
		                                    "playsound minecraft:entity.wither.spawn master @s ~ ~ ~ 10 1.25");
	}
}
