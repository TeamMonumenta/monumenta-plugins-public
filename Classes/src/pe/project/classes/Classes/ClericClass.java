...
import pe.project.classes.Utils.MovementUtil;
import pe.project.classes.Utils.ParticleUtil;
import pe.project.classes.Utils.PlayerUtil;
import pe.project.classes.Utils.PotionUtil;
...
public class ClericClass extends BaseClass {
	...
	public void AbilityOffCooldown(Player player, int abilityID) {
		if (abilityID == CELESTIAL_ID) {
			MessagingUtil.sendActionBarMessage(mPlugin, player, "Celestial Blessing is now off cooldown");
		} else if (abilityID == HEALING_ID) {
			MessagingUtil.sendActionBarMessage(mPlugin, player, "Healing Spell is now off cooldown");
		}
	}
	...
	public void PeriodicTrigger(Player player, boolean twoSeconds, boolean fourtySeconds, boolean sixtySeconds, int originalTime) {
		...
		if (!player.isDead()) {
			...
			//	Rejuvenation
			if (sixSeconds) {
				int rejuvenation = ScoreboardUtil.getScoreboardValue(player, "Rejuvenation");
				if (rejuvenation > 0) {
	...
	public void PlayerDamagedByLivingEntityEvent(Player player, LivingEntity damager, double damage) {
		//	Sanctified
		if (EntityUtil.isUndead(damager)) {
			...
			int sanctified = ScoreboardUtil.getScoreboardValue(player, "Sanctified");
			if (sanctified > 0) {
				double extraDamage = sanctified == 1 ? SANCTIFIED_1_DAMAGE : SANCTIFIED_2_DAMAGE;
				damager.damage(extraDamage);
				
				MovementUtil.KnockAway(player, damager, SANCTIFIED_KNOCKBACK_SPEED);
				
				Location loc = damager.getLocation();
				ParticleUtil.playParticlesInWorld(player.getWorld(), Particle.END_ROD, loc.add(0, 1, 0), 5, 0.35, 0.35, 0.35, 0.001);
	...
	public boolean LivingEntityDamagedByPlayerEvent(Player player, LivingEntity damagee, double damage) {
		//	DivineJustice
		{
			if (PlayerUtil.isCritical(player)) {
				if (EntityUtil.isUndead(damagee)) {
					int divineJustice = ScoreboardUtil.getScoreboardValue(player, "DivineJustice");
					if (divineJustice > 0) {
						damagee.damage(DIVINE_JUSTICE_DAMAGE);
						
						World world = player.getWorld();
						Location loc = damagee.getLocation();
						ParticleUtil.playParticlesInWorld(world, Particle.DRAGON_BREATH, loc.add(0, 1, 0), 50, 0.25, 0.5, 0.5, 0.001);
	...
	public void EntityDeathEvent(Player player, LivingEntity killedEntity) {
		...
		{
			if (PlayerUtil.isCritical(player)) {
				if (EntityUtil.isUndead(killedEntity)) {
					int divineJustice = ScoreboardUtil.getScoreboardValue(player, "DivineJustice");
					if (divineJustice > 1) {
						...
						ParticleUtil.playParticlesInWorld(world, Particle.HEART, loc.add(0, 1, 0), 5, 0.25, 0.25, 0.25, 0.001);
		...
		{
			if (EntityUtil.isUndead(killedEntity)) {
				int heavenlyBoon = ScoreboardUtil.getScoreboardValue(player, "HeavenlyBoon");
				if (heavenlyBoon > 0) {
				...
				if (range <= HEAVENLY_BOON_TRIGGER_RANGE) {
					...
					for(int i = 0; i < entities.size(); i++) {
						...
						if(e instanceof Player) {
							...
							List<PotionEffect> effectList = PotionUtil.getEffects(meta);
							for (PotionEffect effect : effectList) {
								PotionUtil.applyPotion(p, effect);
							}
						}
					}
	...
	public void PlayerShotArrowEvent(Player player, Arrow arrow) {
		if (arrow.isCritical() && player.isSneaking()) {
			int healing = ScoreboardUtil.getScoreboardValue(player, "Healing");
			if (healing > 0) {
				if (!mPlugin.mTimers.isAbilityOnCooldown(player.getUniqueId(), HEALING_ID)) {
					LivingEntity targetEntity = EntityUtil.GetEntityAtCursor(player, HEALING_RANGE, true, true, true);
					if (targetEntity != null && targetEntity instanceof Player) {
						...
						ParticleUtil.playParticlesInWorld(world, Particle.HEART, loc.add(0, 1, 0), 5, 0.35, 0.35, 0.35, 0.001);
						ParticleUtil.playParticlesInWorld(world, Particle.TOTEM, loc.add(0, 1, 0), 5, 0.35, 0.35, 0.35, 0.001);
	...
	public void PlayerInteractEvent(Player player, Action action, Material material) {
		if (player.isSneaking()) {
			if (action == Action.LEFT_CLICK_AIR || action == Action.LEFT_CLICK_BLOCK) {
				int celestial = ScoreboardUtil.getScoreboardValue(player, "Celestial");
				if (celestial > 0) {
					if (!mPlugin.mTimers.isAbilityOnCooldown(player.getUniqueId(), CELESTIAL_ID)) {
						...
						for(int i = 0; i < entities.size(); i++) {
							...
							if(e instanceof Player) {
								...
								ParticleUtil.playParticlesInWorld(world, Particle.VILLAGER_HAPPY, loc.add(0, 1, 0), 15, 0.75, 0.75, 0.75, 0.001);
