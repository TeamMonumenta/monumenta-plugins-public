...
public class MageClass extends BaseClass {
	...
	public void AbilityOffCooldown(Player player, int abilityID) {
		if (abilityID == FROST_NOVA_ID) {
			MessagingUtil.sendActionBarMessage(mPlugin, player, "Frost Nova is now off cooldown");
		} else if (abilityID == MAGMA_SHIELD_ID) {
			MessagingUtil.sendActionBarMessage(mPlugin, player, "Magma Shield is now off cooldown");
		}else if (abilityID == PRISMATIC_SHIELD_ID) {
			MessagingUtil.sendActionBarMessage(mPlugin, player, "Prismatic Shield is now off cooldown");
		}
	}
	...
	public boolean LivingEntityDamagedByPlayerEvent(Player player, LivingEntity damagee, double damage) {
		//	Arcane Strike
		{
			...
			if (InventoryUtil.isWandItem(mainHand)) {
				int arcaneStrike = ScoreboardUtil.getScoreboardValue(player, "ArcaneStrike");
				if (arcaneStrike > 0) {
					if (!mPlugin.mTimers.isAbilityOnCooldown(player.getUniqueId(), ARCANE_STRIKE_ID)) {	
						...
						ParticleUtil.playParticlesInWorld(world, Particle.EXPLOSION_NORMAL, loc.add(0, 1, 0), 50, 2.5, 1, 2.5, 0.001);
						ParticleUtil.playParticlesInWorld(world, Particle.SPELL_WITCH, loc.add(0, 1, 0), 200, 2.5, 1, 2.5, 0.001);
						
						world.playSound(loc, "entity.enderdragon_fireball.explode", 0.5f, 1.5f);
						
						boolean intellectBonus = ScoreboardUtil.getScoreboardValue(player, "Intellect") == 2;
	...
	public void EntityDeathEvent(Player player, LivingEntity killedEntity) {
		int intellect = ScoreboardUtil.getScoreboardValue(player, "Intellect");
		if (intellect > 0) {
	...
	public void PlayerInteractEvent(Player player, Action action, Material material) {
		//	Magma Shield
		{
			...
			if (player.isSneaking()) {
				...
				if (offHand.getType() == Material.SHIELD && (action == Action.RIGHT_CLICK_AIR || action == Action.RIGHT_CLICK_BLOCK)) {
					int magmaShield = ScoreboardUtil.getScoreboardValue(player, "Magma");
					if (magmaShield > 0) {
								...
								public void run() {
									if (++tick == 5) {
										if (player.isBlocking()) {
											...
											for(Entity e : entities) {
												if(e instanceof LivingEntity && !(e instanceof Player)) {
													...
													if (playerDir.dot(toMobVector) > MAGMA_SHIELD_DOT_ANGLE) {
														MovementUtil.KnockAway(player, mob, MAGMA_SHIELD_KNOCKBACK_SPEED);
											...
											ParticleUtil.explodingConeEffect(mPlugin, player, MAGMA_SHIELD_RADIUS, Particle.FLAME, 0.75f, Particle.LAVA, 0.25f, MAGMA_SHIELD_DOT_ANGLE);
		...
		if (action == Action.LEFT_CLICK_AIR || action == Action.LEFT_CLICK_BLOCK) {
			if (player.isSneaking()) {
				ItemStack mainHand = player.getInventory().getItemInMainHand();
				if (InventoryUtil.isWandItem(mainHand)) {
					int frostNova = ScoreboardUtil.getScoreboardValue(player, "FrostNova");
					if (frostNova > 0) {
						if (!mPlugin.mTimers.isAbilityOnCooldown(player.getUniqueId(), FROST_NOVA_ID)) {
							...
							ParticleUtil.playParticlesInWorld(world, Particle.SNOW_SHOVEL, loc.add(0, 1, 0), 400, 4, 1, 4, 0.001);
							ParticleUtil.playParticlesInWorld(world, Particle.CRIT_MAGIC, loc.add(0, 1, 0), 200, 4, 1, 4, 0.001);
	...
	public void LivingEntityShotByPlayerEvent(Player player, Arrow arrow, LivingEntity damagee, EntityDamageByEntityEvent event) {
		//	Elemental Arrows
		{
			int elementalArrows = ScoreboardUtil.getScoreboardValue(player, "Elemental");
	...
	public void PlayerDamagedByLivingEntityEvent(Player player, LivingEntity damager, double damage) {
		//	Prismatic Shield
		{
			...
			if (correctHealth > 0 && correctHealth <= PRISMATIC_SHIELD_TRIGGER_HEALTH) {
				int prismatic = ScoreboardUtil.getScoreboardValue(player, "Prismatic");
				if (prismatic > 0) {
	...
	private void _testItemInHand(Player player) {
		//	Wand Mastery
		{
			int wandMastery = ScoreboardUtil.getScoreboardValue(player, "WandMastery");
			if (wandMastery > 0) {
				ItemStack mainHand = player.getInventory().getItemInMainHand();
