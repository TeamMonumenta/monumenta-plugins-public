...
public class WarriorClass extends BaseClass {
	...
	public void AbilityOffCooldown(Player player, int abilityID) {
		if (abilityID == OBLITERATION_ID) {
			MessagingUtil.sendActionBarMessage(mPlugin, player, "Obliteration is now off cooldown");
		} else if (abilityID == DEFENSIVE_LINE_ID) {
			MessagingUtil.sendActionBarMessage(mPlugin, player, "Defensive Line is now off cooldown");
		}
	}
		...
		if (!(damager instanceof Player)) {
			//	ABILITY: Counter Strike
			{
				...
				if (mRandom.nextFloat() < 0.15f) {		
					int counterStrike = ScoreboardUtil.getScoreboardValue(player, "CounterStrike");
					if (counterStrike > 0) {
	...
	public boolean LivingEntityDamagedByPlayerEvent(Player player, LivingEntity damagee, double damage) {
		//	Obliteration
		{
			...
			if (player.isSneaking() && InventoryUtil.isAxeItem(player.getInventory().getItemInMainHand())) {
				int obliteration = ScoreboardUtil.getScoreboardValue(player, "Obliteration");
				if (obliteration > 0) {
						...
						for(int i = 0; i < entities.size(); i++) {
							...
							if( e != (Entity)player && !(e instanceof Player) && e instanceof LivingEntity ) {
								...
								if (!tags.contains("Elite") && !tags.contains("Boss")) {
									InventoryUtil.removeRandomEquipment(mRandom, mob, BRUTE_FORCE_PIECES_TO_REMOVE);
									...
									ParticleUtil.playParticlesInWorld(world, Particle.EXPLOSION_NORMAL, loc, 100, 1.5, 1.5, 1.5, 0.001);
		...
		//	BRUTE FORCE!!!
		{
			int bruteForce = ScoreboardUtil.getScoreboardValue(player, "BruteForce");
			if (bruteForce > 0) {
				if (PlayerUtil.isCritical(player)) {
					...
					for(int i = 0; i < entities.size(); i++) {
						...
						if( e != (Entity)player && !(e instanceof Player) && e instanceof LivingEntity) {
							...
							MovementUtil.KnockAway(player, mob, BRUTE_FORCE_KNOCKBACK_SPEED);
							
							Location loc = mob.getLocation();
							ParticleUtil.playParticleInWorld(world, Particle.EXPLOSION_LARGE, loc, 1);
						}
					}
				}
			}
	...
	public void PlayerRespawnEvent(Player player) {
		int toughness = ScoreboardUtil.getScoreboardValue(player, "Toughness");
		if (toughness > 0) {
	...
	public void EntityDeathEvent(Player player, LivingEntity killedEntity) {
		int frenzy = ScoreboardUtil.getScoreboardValue(player, "Frenzy");
		if (frenzy > 0) {
		...
		//	Defensive Line
		{
			...
			if (player.isSneaking()) {
				...
				if (offHand.getType() == Material.SHIELD && (action == Action.RIGHT_CLICK_AIR || action == Action.RIGHT_CLICK_BLOCK)) {
					int defensiveLine = ScoreboardUtil.getScoreboardValue(player, "DefensiveLine");
					if (defensiveLine > 0) {
								...
								public void run() {
									if (++tick == 5) {
										if (player.isBlocking()) {
											...
											ParticleUtil.explodingSphereEffect(mPlugin, player, DEFENSIVE_LINE_RADIUS, Particle.FIREWORKS_SPARK, 1.0f, Particle.CRIT, 1.0f);
	...
	private void _testItemInHand(Player player) {
		int weaponMastery = ScoreboardUtil.getScoreboardValue(player, "WeaponMastery");
		if (weaponMastery > 0) {
			ItemStack mainHand = player.getInventory().getItemInMainHand();
			
			//	Player has an axe in their mainhands.
			if (InventoryUtil.isAxeItem(mainHand)) {
			...
			else if (InventoryUtil.isSwordItem(mainHand)) {
			...
			if (InventoryUtil.isPickaxeItem(mainHand)) {
				player.removePotionEffect(PotionEffectType.FAST_DIGGING);
			}
		}
	}
