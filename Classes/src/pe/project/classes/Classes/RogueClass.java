...
import pe.project.classes.Utils.MessagingUtil;
import pe.project.classes.Utils.ParticleUtil;
import pe.project.classes.Utils.PlayerUtil;
...
public class RogueClass extends BaseClass {
	...
	public void AbilityOffCooldown(Player player, int abilityID) {
		if (abilityID == ASSASSINATION_ID) {
			MessagingUtil.sendActionBarMessage(mPlugin, player, "Assassination is now off cooldown");
		} else if (abilityID == FOCUS_ID) {
			MessagingUtil.sendActionBarMessage(mPlugin, player, "Focus is now off cooldown");
		} else if (abilityID == ESCAPE_DEATH_ID) {
			MessagingUtil.sendActionBarMessage(mPlugin, player, "Escape Death is now off cooldown");
		} else if (abilityID == SMOKESCREEN_ID) {
			MessagingUtil.sendActionBarMessage(mPlugin, player, "Smokescreen is now off cooldown");
		}
	}
	...
	public void PlayerInteractEvent(Player player, Action action, Material material) {
		if (action.equals(Action.RIGHT_CLICK_AIR) || (action.equals(Action.RIGHT_CLICK_BLOCK) && !ItemUtils.isInteractable(material))) {
			//	FOCUS
			{
				int focus = ScoreboardUtil.getScoreboardValue(player, "Focus");		
				if (focus > 0) {
					if (_testForSwordsInHand(player)) {
						if (!mPlugin.mTimers.isAbilityOnCooldown(player.getUniqueId(), FOCUS_ID)) {
							...
							ParticleUtil.playParticlesInWorld(world, Particle.SPELL, loc, 75, 1.0, 0.5, 1.0, 0.001);
			...
			if (player.isSneaking()) {
				int smokeScreen = ScoreboardUtil.getScoreboardValue(player, "SmokeScreen");
				if (smokeScreen > 0) {
					if (!mPlugin.mTimers.isAbilityOnCooldown(player.getUniqueId(), SMOKESCREEN_ID)) {
						...
						if (mainHand != null && mainHand.getType() != Material.BOW) {
							...
							Location loc = player.getLocation();
							World world = player.getWorld();
							ParticleUtil.playParticlesInWorld(world, Particle.SMOKE_LARGE, loc.add(0, 1, 0), 150, 2.0, 0.8, 2.0, 0.001);
	...
	public void PlayerDamagedByLivingEntityEvent(Player player, LivingEntity damager, double damage) {
		...
		if (correctHealth > 0 && correctHealth <= ESCAPE_DEATH_HEALTH_TRIGGER) {
			int escapeDeath = ScoreboardUtil.getScoreboardValue(player, "EscapeDeath");
			if (escapeDeath > 0) {
				if (!mPlugin.mTimers.isAbilityOnCooldown(player.getUniqueId(), ESCAPE_DEATH_ID)) {
					...
					ParticleUtil.playParticlesInWorld(world, Particle.SPELL_INSTANT, loc, particles, offset.getX(), offset.getY(), offset.getZ(), 0.001);
					
					if (escapeDeath > 1) {
						ParticleUtil.playParticlesInWorld(world, Particle.CLOUD, loc, particles, offset.getX(), offset.getY(), offset.getZ(), 0.001);
					}
					
					world.playSound(loc, "item.totem.use", 1.0f, 0.5f);
					
					MessagingUtil.sendActionBarMessage(mPlugin, player, "Escape Death has been activated");
	...
	public boolean LivingEntityDamagedByPlayerEvent(Player player, LivingEntity damagee, double damage) {
		if (PlayerUtil.isCritical(player)) {
			ItemStack mainHand = player.getInventory().getItemInMainHand();
			ItemStack offHand = player.getInventory().getItemInOffHand();
			if (InventoryUtil.isSwordItem(mainHand) && InventoryUtil.isSwordItem(offHand)) {
				int assassination = ScoreboardUtil.getScoreboardValue(player, "Assassination");
				if (assassination > 0) {
					if (!mPlugin.mTimers.isAbilityOnCooldown(player.getUniqueId(), ASSASSINATION_ID)) {
						...
						boolean isEliteBoss = EntityUtil.isEliteBoss(damagee);
						if (isEliteBoss) {
						...
						loc.add(0, 1, 0);
						ParticleUtil.playParticlesInWorld(world, Particle.SPELL_MOB, loc, 15, 0.25, 0.5, 0.5, 0.001);
						ParticleUtil.playParticlesInWorld(world, Particle.CRIT, loc, 30, 0.25, 0.5, 0.5, 0.001);
	...
	private void _testForDuelWielding(Player player) {
		int dualWielding = ScoreboardUtil.getScoreboardValue(player, "DualWielding");
		if (dualWielding > 0) {
			if (_testForSwordsInHand(player)) {	
	...
	private boolean _testForSwordsInHand(Player player) {
		...
		return InventoryUtil.isSwordItem(mainHand) && InventoryUtil.isSwordItem(offHand);
	}
	
	private void _viciousCombos(Player player, LivingEntity killedEntity) {
		if (EntityUtil.isEliteBoss(killedEntity)) {
			int viciousCombos = ScoreboardUtil.getScoreboardValue(player, "ViciousCombos");
			if (viciousCombos > 0) {
				World world = player.getWorld();
				...
				if (viciousCombos > 1) {
					...
					MessagingUtil.sendActionBarMessage(mPlugin, player, "All your cooldowns have been reset");
				...
				ParticleUtil.playParticlesInWorld(world, Particle.SWEEP_ATTACK, loc, 350, VICIOUS_COMBOS_RANGE, VICIOUS_COMBOS_RANGE, VICIOUS_COMBOS_RANGE, 0.001);
				ParticleUtil.playParticlesInWorld(world, Particle.SPELL_MOB, loc, 350, VICIOUS_COMBOS_RANGE, VICIOUS_COMBOS_RANGE, VICIOUS_COMBOS_RANGE, 0.001);
			}
		}
	}
}
